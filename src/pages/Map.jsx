...
export default function MapPage() {
  ...
  const generate = () => {
    ...
    // Resolve from/to terminal IDs from chosen coordinates so we don't inherit
    // suggestions[0]'s "t1 → t5" (Centro → Sambaqui) on every generated route.
    const matchTerminal = (c) =>
      terminals.find((t) => t.coords[0] === c[0] && t.coords[1] === c[1])?.id ?? null;
    store.setRoute({
      ...best,
      id: `gen-${Date.now()}`,
    ...
    <AppShell>
      ...
      <div className="relative px-4 pt-12" style={{ zIndex: 1000 }}>
        <div className="glass rounded-2xl shadow-card border border-white/40 flex items-center gap-2 p-2">
          <button onClick={() => navigate("/home")} className="size-9 grid place-items-center rounded-xl bg-white/60">
