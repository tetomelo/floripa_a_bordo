    ...
    <AppShell>
      ...
      <section className="px-5">
        <h2 className="font-semibold text-sm mb-2 flex items-center gap-2">
          <Heart className="size-4 text-destructive" /> Rotas salvas
        </h2>
        {favRoutes.length === 0 ? (
            ...
            {favRoutes.map((s) => {
              ...
              return (
                <div key={s.id} className="rounded-2xl bg-card border border-border p-4 shadow-card flex items-center gap-3">
                  ...
                  <button onClick={() => { store.setRoute(s); navigate("/route-result"); }} className="flex-1 text-left">
      ...
      <section className="px-5 mt-6">
        <h2 className="font-semibold text-sm mb-2 flex items-center gap-2">
          <Clock className="size-4 text-primary" /> Histórico de viagens
        </h2>
        <div className="space-y-2">
