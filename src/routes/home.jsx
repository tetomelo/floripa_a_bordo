    ...
    return (<AppShell>
      ...
      <div className="px-5">
        <div className="glass rounded-2xl shadow-card border border-border flex items-center gap-2 px-4 py-3">
          ...
          <input placeholder="Para onde vamos navegar?" className="bg-transparent outline-none text-sm flex-1" onFocus={() => navigate({ to: "/map" })}/>
        </div>
      </div>

      {/* Map preview */}
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mt-4 mx-5 h-64 rounded-3xl overflow-hidden shadow-float relative">
      ...
      <section className="px-5 mt-6">
        ...
        <div className="space-y-3">
          {suggestions.map((s) => {
            ...
            return (<motion.button whileTap={{ scale: 0.98 }} key={s.id} onClick={() => { store.setRoute(s); navigate({ to: "/route-result" }); }} className="w-full text-left rounded-2xl bg-card border border-border p-4 shadow-card flex items-center gap-3">
      ...
      {/* Recent trips */}
      {recentTrips.length > 0 && (<section className="px-5 mt-6">
          <div className="flex items-center justify-between mb-3">
      ...
      <section className="px-5 mt-6">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 -mx-5 px-5 snap-x">
          {terminals.slice(0, 5).map((t) => (<div key={t.id} className="min-w-[180px] snap-start rounded-2xl bg-gradient-ocean text-white p-4 shadow-card">
