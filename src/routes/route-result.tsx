...
export const Route = createFileRoute("/route-result")({
    head: () => ({ meta: [{ title: "Rota — Floripa a Bordo" }] }),
    ...
    return (<AppShell>
      <div className="relative h-[55vh]">
        <ClientOnly fallback={<div className="size-full bg-ocean-light animate-pulse"/>}>
          <FloripaMap path={route.path} origin={route.path[0]} destination={route.path[route.path.length - 1]} showTerminals={false} interactive={false}/>
        </ClientOnly>
        <button onClick={() => navigate({ to: "/map" })} className="absolute top-12 left-4 size-10 glass rounded-full grid place-items-center shadow-card">
          <ArrowLeft className="size-5"/>
        </button>
        <button onClick={() => store.toggleFav(route.id)} className={`absolute top-12 right-4 size-10 rounded-full grid place-items-center shadow-card ${isFav ? "bg-destructive text-white" : "glass"}`}>
          <Heart className={`size-5 ${isFav ? "fill-current" : ""}`}/>
        </button>
      </div>

      <motion.div initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative -mt-8 bg-background rounded-t-[2.5rem] px-5 pt-6 pb-6 shadow-float">
        ...
        <div className="mt-2 space-y-2">
          {suggestions.filter((s) => s.id !== route.id).slice(0, 3).map((s) => (<button key={s.id} onClick={() => store.setRoute(s)} className="w-full flex items-center gap-3 p-3 rounded-2xl border border-border bg-card">
