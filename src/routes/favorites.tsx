...
function Favorites() {
    ...
    return (<AppShell>
      ...
      <section className="px-5">
        ...
        {favRoutes.length === 0 ? (<div className="rounded-2xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
            ...
            {favRoutes.map((s) => {
                ...
                return (<div key={s.id} className="rounded-2xl bg-card border border-border p-4 shadow-card flex items-center gap-3">
      ...
      <section className="px-5 mt-6">
        ...
        <div className="space-y-2">
          {suggestions.slice(0, 3).map((s, i) => {
            const from = terminals.find((t) => t.id === s.from)?.name;
            const to = terminals.find((t) => t.id === s.to)?.name;
            return (<button key={s.id} onClick={() => { store.setRoute(s); navigate({ to: "/route-result" }); }} className="w-full text-left rounded-2xl bg-secondary p-4 flex items-center gap-3">
