...
import { suggestions, terminals, FLORIPA_CENTER, type Terminal } from "@/lib/mock-data";
...
function MapPage() {
  ...
  const [origin, setOrigin] = useState<Point | null>({ coords: FLORIPA_CENTER, label: "Centro de Florianópolis" });
  const [destination, setDestination] = useState<Point | null>(null);
  const [mode, setMode] = useState<"origin" | "destination">("destination");
  ...
  const matches = useMemo<Terminal[]>(() => {
  ...
  const handleClick = (c: [number, number]) => {
  ...
  const selectTerminal = (t: Terminal) => {
  ...
  return (
    <AppShell>
      ...
      <div className="relative px-4 pt-12" style={{ zIndex: 1000 }}>
        <div className="glass rounded-2xl shadow-card border border-white/40 flex items-center gap-2 p-2">
          ...
          <div className="flex-1 space-y-1">
            <InputRow
              icon={<MapPin className="size-3.5 text-aqua" />}
              label="Origem"
              placeholder="De onde você sai?"
              value={originQuery}
              active={mode === "origin"}
              onFocus={() => { setMode("origin"); setFocused("origin"); }}
              onChange={(v) => { setOriginQuery(v); if (!v) setOrigin(null); }}
              onClear={() => { setOriginQuery(""); setOrigin(null); }}
            />
            <InputRow
              icon={<Navigation2 className="size-3.5 text-primary" />}
              label="Destino"
              placeholder="Para onde vamos navegar?"
              value={destQuery}
              active={mode === "destination"}
              onFocus={() => { setMode("destination"); setFocused("destination"); }}
              onChange={(v) => { setDestQuery(v); if (!v) setDestination(null); }}
              onClear={() => { setDestQuery(""); setDestination(null); }}
            />
          </div>
        </div>
        ...
        {focused && (
          <div className="mt-2 glass rounded-2xl shadow-card border border-white/40 overflow-hidden max-h-72 overflow-y-auto">
            ...
            {matches.length === 0 ? (
              ...
              matches.map((t) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2.5 flex items-center gap-3 hover:bg-white/60 transition-colors border-b border-white/20 last:border-0"
                >
          ...
          <div className="mt-3 flex gap-2 overflow-x-auto">
            {terminals.slice(0, 4).map((t) => (
              <button
                ...
                onClick={() => {
                  const point = { coords: t.coords, label: t.name };
                  if (mode === "origin") { setOrigin(point); setOriginQuery(t.name); }
                  else { setDestination(point); setDestQuery(t.name); }
                }}
      ...
      <motion.div
        ...
        className="absolute bottom-28 inset-x-4 space-y-2"
        style={{ zIndex: 1000 }}
      >
        ...
        <button
          ...
          className="w-full bg-gradient-ocean text-white font-semibold rounded-full py-4 shadow-float flex items-center justify-center gap-2 disabled:opacity-50"
        >
...
function InputRow({
  icon, label, placeholder, value, active, onFocus, onChange, onClear,
}: {
  ...
  return (
    <div className={`flex items-center gap-2 rounded-xl px-3 py-2 transition-colors ${active ? "bg-white/80" : "bg-white/40"}`}>
      {icon}
      <div className="flex-1 min-w-0">
        ...
        <input
          ...
          className="w-full bg-transparent outline-none text-sm font-medium placeholder:text-muted-foreground/70"
        />
      </div>
      {value && (
        <button onClick={onClear} className="size-6 grid place-items-center rounded-full bg-white/60 text-muted-foreground">
          <X className="size-3" />
        </button>
      )}
    </div>
...
function coordLabel([lat, lng]: [number, number]) {
