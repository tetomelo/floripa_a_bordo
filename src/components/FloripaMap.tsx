...
function Recenter({ path }: { path?: [number, number][] }) {
  const map = useMap();
  useEffect(() => {
...
function ClickCapture({ onClick }: { onClick?: (c: [number, number]) => void }) {
  const map = useMap();
...
export default function FloripaMap({
  path, origin, destination, showTerminals = true,
  height = "100%", interactive = true, onMapClick,
}: Props) {
  return (
    <MapContainer
    ...
    >
      ...
      {showTerminals && terminals.map((t) => (
        <Marker key={t.id} position={t.coords} icon={terminalIcon}>
          <Popup>
      ...
      {path && path.length > 1 && (
        <Polyline
          positions={path}
          pathOptions={{ color: "oklch(0.42 0.15 240)", weight: 5, opacity: 0.85, dashArray: "10 8" }}
        />
      )}
